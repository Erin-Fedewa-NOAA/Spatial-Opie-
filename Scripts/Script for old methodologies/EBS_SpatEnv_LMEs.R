...
setwd("C:/Users/erin.fedewa/Work/Research/Spatial Opie MS/Datasets")
...
#2)Re-fit lme random slope and gls full models with selected correlation structure and compare w/ AICc/REML
  ...
  AICc(cm1, cm2, cm3) #Fixed effects model is supported by AICc
...
#3) Test for best-supported fixed structure using likelihood ratio test comparing nested models fit with ML
  ...
  cm6<-gls(center.D95~1+DFA_Trend, method="ML", correlation=corAR1(form = ~1|SIZESEX), data = dat)
    AICc(cm5,cm6) #model without abundance is supported, though marginal difference in AIC scores 
...
#4) Re-fit final model with REML estimation 
    ...
    mod_final <-gls(center.D95~1+DFA_Trend, method="REML", correlation=corAR1(form = ~1|SIZESEX), data = dat)
      ...
      rcompanion::nagelkerke (mod_final)
...
ggsave(g2, filename = "modelfit.png", device = "png", width = 7, height = 5, 
       dpi = 300)
